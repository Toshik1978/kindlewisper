...
class MOBIFile:
    def __init__(self, path, kindle, config, sequence_number, title, asin, position, mode, cloud, cover_size):
        self.mode = mode
        self.config = config
        self.path = path
        self.kindle = kindle
        ...
        if (self.mode == 'reader'):
            ...
            if self.kindle.need_cover:
                if cover != '':
                    try:
                        ...
                        if self.cover_size == 'pw':
                            ...
                            if self.cover_size == 'kv':
                                ready_cover = ready_cover.resize((303, 455), Image.ANTIALIAS)
                            else:
                                ready_cover = ready_cover.resize((333, 500), Image.ANTIALIAS)
                            ...
                            if (self.write_thumb):
                                ...
                                if self.cover_size == 'pw':
                                    ...
                                    if self.cover_size == 'kv':
                                        ...
                                        ready_cover = ready_cover.resize((333, 500), Image.ANTIALIAS)
                                ready_cover = ready_cover.convert('L')
                                self.txt2defaultcover(self.title, self.author, self.seqnumber, ready_cover,
                                                      self.position)
                    else:  # extract cover
                        try:
                            # ready_cover = self.get_cover_image()
                ...
                if self.kindle.ssh:
                    ...
                    ssh = Popen(
                        '"' + self.config['SSH']['PSCPPath'] + '" "' + tmp_cover + '" root@' + self.kindle.path +
                    ...
                    if (self.write_thumb):
                        if (cloud == 'no'):
                            ready_cover.save(os.path.join(self.kindle.path, 'system',
                                                          'thumbnails',
                                                          'thumbnail_' + self.asin + '_EBOK_portrait.jpg'), 'JPEG')
                        else:
                            # get ASIN from file
                            section = KindleUnpack.Sectionizer(self.path)
                            mhlst = [KindleUnpack.MobiHeader(section, 0)]
                            ...
                            if assassin == None:
                                assassin = 'None'
                            ready_cover.save(os.path.join(self.kindle.path, 'system',
                                                          'thumbnails', 'thumbnail_' + assassin + '_PDOC_portrait.jpg'),
                                             'JPEG')
            ...
            if source_size < self.kindle.get_free_space():
                if self.kindle.ssh:
                    ...
                    if cloud == 'no':
                        saved = 0
                        if directory == None:
                            target = open(os.path.join(self.kindle.path, 'documents', os.path.basename(self.path)),
                                          'wb')
                        else:
    ...
    def get_seqnumber(self, infilename, seqnumber):
        ...
        result = seqnumber if seqnumber != 'auto' else infilename[0] + (
            infilename[1] if infilename[1].isdigit() else "")
    ...
    def txt2img(self, title, seqnum, img, position):
        try:
            ...
            if title is not None:
                ...
                if seqnum is not None:
                    if self.cover_size == "pw":
                        draw.line((xmask + 0.5 * haxis, ymask, xmask + haxis + 0.7 * width, ymask), fill=bgcolor,
                                  width=int(1.4 * haxis))
                        titlelength = 12
                    else:
                        draw.line((xmask + 0.5 * haxis, ymask, xmask + 1.5 * haxis + 0.7 * width, ymask), fill=bgcolor,
                                  width=int(1.4 * haxis))
                        titlelength = 22
                    margin = xmask + haxis
                else:
        ...
        for i in range(beg, end):
            ...
            if len(imgnames) - 1 == coverid:
                ...
                if self.cover_size == 'pw':
                    ...
                    if self.cover_size == 'kv':
                        cover = cover.resize((303, 455), Image.ANTIALIAS)
                    else:
                        cover = cover.resize((333, 500), Image.ANTIALIAS)
